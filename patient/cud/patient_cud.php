    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if($_POST["operation"] == $i){
            $sql0 = "INSERT INTO Patient (FName, MInit, LName, Street, City, State, Zip_Code, Primary_Physician_ID) 
                    VALUES ('$patfname', '$patminit', '$patlname', '$patstreet', '$patcity', '$patstate', '$patzip', ?)";
            $stmt0 = $conn->prepare($sql0);
            $stmt0->bind_param("i", $patdocid);
            $stmt0->execute();
            ...
            if ($patinsid !== null) {
                ...
                $stmt_new_patid = $conn->prepare($sql_get_new_facid);
                $stmt_new_patid->execute();
                $result = $stmt_new_patid->get_result();
                ...
                $stmt1->bind_param("ii", $latestPatientID, $patinsid);
                $stmt1->execute();
            }
        ...
        if($_POST["operation"] == $u){
            ...
            if ($patstreet !== null || $patcity !== null || $patstate !== null || $patzip !== null || $patname !== null || $patminit !== null || $patlname !== null || $patdocid !== null) {
                ...
                if ($patdocid !== null) {
                    $stmt_update_main->bind_param("ii", $patdocid, $patient_id);
                }
                else {
                    $stmt_update_main->bind_param("i", $patient_id);
                }
                $stmt_update_main->execute();
            }
            if ($patinsid !== null) {
                $sql_update_insured_by = "UPDATE Insured_By SET Insurer_ID = ? WHERE Patient_ID = ?";
                $stmt_update_insured_by = $conn->prepare($sql_update_insured_by);
                $stmt_update_main->bind_param("ii", $patinsid, $patient_id);
        ...
        if ($_POST["operation"] == $d) {
            ...
            $sql_delete_main = "DELETE FROM Patient WHERE Patient_ID = ?";
            $stmt_del_main_rec = $conn->prepare($sql_delete_main);
            $stmt_del_main_rec->bind_param("ii", $patinsid, $patient_id);
